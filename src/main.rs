...
impl App {
    ...
    fn show(&mut self) -> Task<cosmic::Action<Msg>> {
        ...
        if !self.visible {
            ...
            let mut tasks: Vec<Task<cosmic::Action<Msg>>> = outputs
                .into_iter()
...
impl Application for App {
    ...
    fn update(&mut self, message: Msg) -> Task<cosmic::Action<Msg>> {
        match message {
            ...
            Msg::LauncherEvent(event) => match event {
                subscriptions::launcher::Event::Started(tx) => {
                    self.tx = Some(tx);
                }
            ...
            Msg::Activate(index) => {
                let index = index.unwrap_or(self.focused);
